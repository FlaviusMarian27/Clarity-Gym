  ...
  <nav class="fixed top-0 left-0 right-0 z-50 bg-card shadow-sm">
    <div class="w-full px-4 md:px-8 py-4 flex justify-between items-center">
      ...
      <div class="hidden lg:flex items-center gap-6">
        ...
        <span @click="navigateTo('antrenori')" class="text-text hover:text-primary transition-colors duration-200 font-medium cursor-pointer">Antrenori</span>
        <span @click="navigateTo('abonamente')" class="text-text hover:text-primary transition-colors duration-200 font-medium cursor-pointer">Abonamente</span>
        ...
        <a href="#antrenori" class="relative" @click="handleBellClick">
          ...
          <span v-if="notifCount > 0" class="absolute -top-1 -right-1 w-5 h-5 bg-red-400 text-white text-xs rounded-full flex items-center justify-center font-bold">{{ notifCount }}</span>
        </a>

        <!-- Avatar -->
        <div class="relative">
          <button @click="toggleDropdown" class="w-10 h-10 rounded-full bg-primary text-white font-bold text-lg flex items-center justify-center hover:bg-secondary transition-colors duration-200 overflow-hidden">
      ...
      <div class="flex lg:hidden items-center gap-3">
        <a href="#antrenori" class="relative" @click="handleBellClick">
          ...
          <span v-if="notifCount > 0" class="absolute -top-1 -right-1 w-4 h-4 bg-red-400 text-white text-xs rounded-full flex items-center justify-center font-bold">{{ notifCount }}</span>
        </a>
        <button @click="toggleDropdown" class="w-9 h-9 rounded-full bg-primary text-white font-bold flex items-center justify-center overflow-hidden">
          <img v-if="avatarUrl" :src="avatarUrl.startsWith('http') ? avatarUrl : 'http://localhost:8080' + avatarUrl" class="w-full h-full object-cover" />
    ...
    <div v-if="menuOpen" class="lg:hidden bg-card border-t border-bg px-4 py-4 flex flex-col gap-2">
      ...
      <span @click="navigateTo('antrenori'); menuOpen = false" class="text-text hover:text-primary font-medium cursor-pointer py-2">Antrenori</span>
      <span @click="navigateTo('abonamente'); menuOpen = false" class="text-text hover:text-primary font-medium cursor-pointer py-2">Abonamente</span>
